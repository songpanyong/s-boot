...
@Slf4j
@Component
public class BankCallbackMinuteTaskJob {
	...
	@Scheduled( fixedDelayString = "${jobs.bankCallbackMinute.schedule:600000}")
	public void find() {
		try {
			...
			if (null != concurrentManager) {
				...
				for (CallBackInfo callBackInfo : list) {
					bankCallSyncBackService.bankCallBack(callBackInfo,"2");
				}
			}
		} catch (Exception e) {
