...
public class PlatformInfoQueryService {
	...
	public PlatformInfoResponse platformInfo(String userOid) {
		...
		if (entity != null) {
			...
			// 查询平台账户信息
			List<String> openList = platformAccountInfoDao.findByUserOidAndStatus(entity.getUserOid(),
					PlatformAccountInfoEntity.STATUS_RUN);
			List<String> closeList = platformAccountInfoDao.findByUserOidAndStatus(entity.getUserOid(),
					PlatformAccountInfoEntity.STATUS_STOP);
			resp.setOpenList(openList);
			resp.setCloseList(closeList);
		}
		String settleStatus = ApplyAuditTypeEnum.AUDIT_NO;
		// 查询平台是否有待审核信息
		PlatformInfoAuditEntity platformInfoAuditEntity = platformInfoAuditDao
				.findAuditingByUserOid(entity.getUserOid());
	...
	public List<PlatformAccountInfoEntity> getChangeAccountInfoList(AccountInfoForm req) {
		...
		if (req.getAccountStatus() == null || "".equals(req.getAccountStatus())) {
			platformAccountList = platformAccountInfoDao.findByUserOidAndType(req.getUserOid(), req.getAccountType());
		} else {
			platformAccountList = platformAccountInfoDao.findByUserOidAndTypeAndStatus(req.getUserOid(),
					req.getAccountType(), req.getAccountStatus());
		}
