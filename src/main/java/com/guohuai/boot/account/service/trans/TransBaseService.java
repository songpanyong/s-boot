...
import com.guohuai.boot.account.service.AccountTradeService;
...
import com.guohuai.component.util.TransferOrderTypeEnum;
import com.guohuai.component.util.UserTypeEnum;

import lombok.extern.slf4j.Slf4j;

/**
 * @Description: 账户交易基本服务
 * @author ZJ
 * @date 2018年1月19日 下午3:37:15
 * @version V1.0
 */
@Slf4j
@Service
public class TransBaseService {
	@Autowired
	...
	/**
	 * 发行人收款、放款交易 收款,orderType传58 放款,orderType传57 @Title:
	 * publisherLoan @param @param req @param @return @return
	...
	public AccountTransResponse publisherTrans(AccountTransRequest req) {
		...
		if (Constant.SUCCESS.equals(orderResp.getReturnCode())) {
			...
			// 查询发行人可用金户
			AccountInfoEntity usableAccount = accountInfoDao.findByUserOidAndAccountTyp(req.getUserOid(),
			...
			if (orderType.equals(OrderTypeEnum.PUBLISHERRECE.getCode())) {
				...
				if (orderMoney.compareTo(usableAccountBalance) > 0) {
					resp.setReturnCode(Constant.BALANCELESS);
					resp.setErrorMessage("发行人可用金户余额不足!");
					log.info("发行人可用金户余额不足!userOid = " + req.getUserOid());
					return resp;
				}
				log.info("扣除发行人可用金户余额:{}", orderMoney);
				// 扣除发行人可用金户余额
				int updateUsableAccountResult = accountInfoDao.subtractBalance(orderMoney,
						usableAccount.getAccountNo());
				// 增加发行人基本户余额
				log.info("增加发行人基本户余额:{}", orderMoney);
				if (updateUsableAccountResult == 0) {
					log.error("扣除发行人可用金户余额，发行人可用金户余额不足！");
					throw new GHException(Integer.parseInt(Constant.BALANCELESS), "扣除发行人可用金户余额失败，发行人可用金户余额不足！");
				}
				...
				log.info("发行人收款账户更新结果：updateUsableAccountResult={},updateBasicAccountResult={}",
						updateUsableAccountResult, updateBasicAccountResult);

				BigDecimal usableAccountAfterBalance = BigDecimal.ZERO;
				BigDecimal basicAccountAfterBalance = BigDecimal.ZERO;

				usableAccountAfterBalance = usableAccountBalance.subtract(orderMoney);
				...
				transResp = accountTradeService.addTrans(req, orderResp.getOrderOid(), basicAccount.getAccountNo(),
						usableAccount.getAccountNo(), req.getUserOid(), UserTypeEnum.PUBLISHER.getCode(),
						basicAccountAfterBalance, "01", AccountTypeEnum.BASICER.getCode(), basicAccount.getAccountNo());

				req.setOrderDesc("发行人收款可用金户明细");
				transResp = accountTradeService.addTrans(req, orderResp.getOrderOid(), basicAccount.getAccountNo(),
						usableAccount.getAccountNo(), req.getUserOid(), UserTypeEnum.PUBLISHER.getCode(),
						usableAccountAfterBalance, "02", AccountTypeEnum.AVAILABLE_AMOUNT.getCode(),
						usableAccount.getAccountNo());

				orderEntity.setInputAccountNo(basicAccount.getAccountNo());
				orderEntity.setOutpuptAccountNo(usableAccount.getAccountNo());
				...
				if (rechargeFrozenAccount != null
					...
					if (updateRechargeFrozenAccountResult == 0) {
						...
						transResp = accountTradeService.addTrans(req, orderResp.getOrderOid(),
								usableAccount.getAccountNo(), rechargeFrozenAccount.getAccountNo(), req.getUserOid(),
				...
				// 增加发行人可用金户余额
				log.info("增加发行人可用金户余额:{}", orderMoney);
				int updateUsableAccountResult = accountInfoDao.addBalance(orderMoney, usableAccount.getAccountNo());
				log.info(
						"发行人放款账户更新结果：updateUsableAccountResult={},updateBasicAccountResult={},updateRechargeFrozenAccountResult={}",
						updateUsableAccountResult, updateBasicAccountResult, updateRechargeFrozenAccountResult);
				if (updateUsableAccountResult == 0) {
					log.error("增加发行人可用金户余额失败！");
					throw new GHException(Integer.parseInt(Constant.BALANCELESS), "增加发行人可用金户余额失败！");
				}
				...
				usableAccountAfterBalance = usableAccountBalance.add(orderMoney);
				/**
				 * 插入交易流水
				 */
				req.setOrderDesc("发行人放款可用金户明细");
				transResp = accountTradeService.addTrans(req, orderResp.getOrderOid(), usableAccount.getAccountNo(),
						basicAccount.getAccountNo(), req.getUserOid(), UserTypeEnum.PUBLISHER.getCode(),
						usableAccountAfterBalance, "01", AccountTypeEnum.AVAILABLE_AMOUNT.getCode(),
						usableAccount.getAccountNo());
				...
				orderEntity.setInputAccountNo(usableAccount.getAccountNo());
	...
	/**
	 * 请求req转换订单req @Title: convertTransRequest @param @param req @param @param
	 * ordReq @return void @throws
	 */
	...
	/**
	 * 会员账户交易 @Title: addAccountTrans @Description: @param @param
	...
	/**
	 * 批量发行人产品户增加余额方法 @Title: trade @Description: dataList @param @return @return
	 * AccountTransResponse @throws
	 */
	@Transactional
	...
	/**
	 * 平台、投资人转账 @Title: transferAccount @Description: @param @param
	 * req @param @return @return TransferAccountResponse @throws
	 */
	...
	/**
	 * 平台、投资人入账 @Title: enterAccount @Description: @param @param
	 * req @param @return @return EnterAccountResponse @throws
	 */
	...
	/**
	 * 判断类型是否存在 @Title: checkTransferRequest @Description: @param @param
	 * req @param @return @return AccountTransResponse @throws
	 */
	...
	/**
	 * @Title: checkEnterAccountRequest @Description: 判断类型是否存在 @param @param
	 *         req @param @return @return EnterAccountResponse @throws
	 */
	...
	/**
	 * 生成对账文件 @Title: buildTransFile @Description: req @param @throws
	...
	public String buildTransFile(TransDetailQueryRequest req) throws Exception {
		...
		try {
			...
			TransDetailListResponse resp = this.transQueryService.tansDetailQueryList(req);
	...
	public CreateTransResponse addTransferTrans(String requestNo, String orderNo, String orderType,
			...
			String orderDesc, String direction, AccountInfoEntity accountInfoEntity) {
