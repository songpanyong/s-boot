...
/**
 * @ClassName: AccountNettingService
 * @Description: 账户轧差结算
 * @author CHENDONGHUI
 * @date 2017年6月6日 下午 15:38:39
 */
@Slf4j
@Service
@Transactional
public class AccountNettingService {
	 ...
	 * 订单轧差，校验金额
	 * 锁定发行人可用金户，判断是否可进行结算
	 * 申购>赎回，发行人归集户-->发行人可用金户
	 * 赎回>申购，发行人可用金户-->发行人归集户
	 * 操作赎回订单账户
	...
	public AccountSettlementResponse nettingSettlement(AccountSettlementRequest req) {
		...
		if(!StringUtil.isEmpty(userOid)){
			...
			if (userInfo != null) {
				...
				availableAccount = accountInfoDao.findAccountByAccountTypeAndUserOid(
						AccountTypeEnum.AVAILABLE_AMOUNT.getCode(), userOid);
		...
		if(!(OrderTypeEnum.ABAPPLY.getCode().equals(req.getOrderType())||OrderTypeEnum.ABREDEEM.getCode().equals(req.getOrderType()))){
		...
		if(nettingBalance.compareTo(BigDecimal.ZERO) == 0){//申购等于赎回
			...
			resp = this.applyMoreThanRedeem(resp, transResp, nettingBalance, availableBalance,
					userOid, availableAccount, accOrderEntity, redeemBalance);
			...
			if(availableBalance.compareTo(nettingBalance) >= 0){
				resp = this.redeemMoreThanApply(resp, transResp, nettingBalance, availableBalance,
						userOid, availableAccount, accOrderEntity, redeemBalance);
			}else{
				log.info("发行人可用金不足 ,发行人Uid:{},发行人可用金账户:{},可用金：{}，轧差额{}",
						userOid,availableAccount.getAccountNo(),availableBalance,nettingBalance);
	...
	/**
	 * 申购等于赎回，需冻结参与轧差赎回单总金额
	 * 先判断归集户余额是否充足，
	 * 若充足，扣除发行人归集户赎回单总金额，增加发行人资金冻结户余额
	 * 若不足，扣除发行人可用金户余额补足赎回单总额到发行人资金冻结户余额
	 * 可用金不足，轧差失败
	 * @param resp
	 * @param transResp
	...
	public AccountSettlementResponse applyEquelRedeem(AccountSettlementResponse resp, CreateTransResponse transResp, 
		...
		AccountInfoEntity settlementAccount = accountInfoDao.findAccountByAccountTypeAndUserOid(
				AccountTypeEnum.COLLECTION_SETTLEMENT.getCode(), userOid);
		...
		AccountInfoEntity availableAccount = accountInfoDao.findAccountByAccountTypeAndUserOid(
				AccountTypeEnum.AVAILABLE_AMOUNT.getCode(), userOid);
		if(availableAccount == null){
			resp.setReturnCode(Constant.ACCOUNTNOTEXISTS);
		...
		AccountInfoEntity frozenAccount = accountInfoDao.findAccountByAccountTypeAndUserOid(
				AccountTypeEnum.REDEEMFROZEN.getCode(), userOid);
		...
		if(settlementAccountBalance.compareTo(redeemBalance) >= 0){
			...
			//发行人归集户转账给发行人资金冻结户
			baseResp = accountTransferService.publisherSettlementAccountTransferToPublisherFrozenAccount(accOrderEntity, 
					userOid, settlementAccountBalance, settlementAccountBalance, settlementAccount, frozenBalance, frozenAccount);
			//发行人可用金户转账给发行人资金冻结户
			if(Constant.SUCCESS.equals(baseResp.getReturnCode())){
				baseResp = accountTransferService.publisherAvailableAccountTransferToPublisherFrozenAccount(accOrderEntity, 
						userOid, difference, availableBalance, availableAccount, frozenBalance, frozenAccount);
			}
		}
		if(Constant.SUCCESS.equals(baseResp.getReturnCode())){
	...
	public AccountSettlementResponse applyMoreThanRedeem(AccountSettlementResponse resp, CreateTransResponse transResp, 
		...
		if(settlementAccountBalance.compareTo(nettingBalance.add(redeemBalance))>=0){
			//发行人归集户向发行人可用金转账
			baseResp = accountTransferService.publisherSettlementAccountTransferToPublisherAvailableAccount(accOrderEntity, 
					userOid, nettingBalance, settlementAccountBalance, settlementAccount, availableBalance, availableAccount);
			//发行人归集户向发行人资金冻结户转账
			if(Constant.SUCCESS.equals(baseResp.getReturnCode())){
				baseResp = accountTransferService.publisherSettlementAccountTransferToPublisherFrozenAccount(accOrderEntity, 
						userOid, redeemBalance, settlementAccountBalance, settlementAccount, frozenBalance, frozenAccount);
			...
			if(difference.compareTo(nettingBalance) > 0){
				...
				if(Constant.SUCCESS.equals(baseResp.getReturnCode())){
					baseResp = accountTransferService.publisherAvailableAccountTransferToPublisherFrozenAccount(accOrderEntity, 
							userOid, redeemBalance.subtract(settlementAccountBalance), availableBalance, availableAccount, frozenBalance, frozenAccount);
				}
				...
				//发行人归集户向发行人可用金户转账
				baseResp = accountTransferService.publisherSettlementAccountTransferToPublisherAvailableAccount(accOrderEntity, 
						userOid, settlementAccountBalance.subtract(redeemBalance), settlementAccountBalance, settlementAccount, availableBalance, availableAccount);
				//发行人归集户向发行人资金冻结户转账
				if(Constant.SUCCESS.equals(baseResp.getReturnCode())){
					baseResp = accountTransferService.publisherSettlementAccountTransferToPublisherFrozenAccount(accOrderEntity, 
							userOid, redeemBalance, settlementAccountBalance, settlementAccount, frozenBalance, frozenAccount);
	...
	public AccountSettlementResponse redeemMoreThanApply(AccountSettlementResponse resp, CreateTransResponse transResp, 
		...
		if(redeemBalance.compareTo(nettingBalance) > 0){
			...
			if(settlementAccountBalance.compareTo(difference) >= 0){
				//发行人可用金户向发行人资金冻结户转账
				baseResp = accountTransferService.publisherAvailableAccountTransferToPublisherFrozenAccount(accOrderEntity, 
						userOid, nettingBalance, availableBalance, availableAccount, frozenBalance, frozenAccount);
				//发行人归集户向发行人资金冻结户转账
				if(Constant.SUCCESS.equals(baseResp.getReturnCode())){
					baseResp = accountTransferService.publisherSettlementAccountTransferToPublisherFrozenAccount(accOrderEntity, 
							userOid, difference, settlementAccountBalance, settlementAccount, frozenBalance, frozenAccount);
				}
			}else{
				difference = redeemBalance.subtract(settlementAccountBalance);
				//发行人可用金户向发行人资金冻结户转账
	 ...
	 * @return
	 */
//	private CreateTransResponse addTransWithOrder(String inputAccountNo, String outpuptAccountNo,
//			String userOid, BigDecimal orderBalance, BigDecimal afterBalance, String remark, String orderDesc,
//			String direction, AccOrderEntity accOrderEntity) {
...
//		transRequest.setOrderBalance(orderBalance);
//		transRequest.setVoucher(BigDecimal.ZERO);
//		transRequest.setTransTime(new Timestamp(System.currentTimeMillis()));
//		transRequest.setDataSource(accOrderEntity.getSystemSource());
...
//	private CreateTransResponse addTrans(String inputAccountNo, String outpuptAccountNo,
//			String userOid, BigDecimal orderBalance, BigDecimal afterBalance, String remark, String orderDesc,
//			String direction) {
//		CreateTransRequest transRequest = new CreateTransRequest();
//		String reqNo = "T2REQ"+System.currentTimeMillis();
//		String orderNo = "T2ORDER"+System.currentTimeMillis();
//		transRequest.setUserOid(userOid);
//		transRequest.setAccountOid(inputAccountNo);
//		transRequest.setUserType("T2");
//		transRequest.setRequestNo(reqNo);
//		transRequest.setAccountOrderOid(orderNo);
//		transRequest.setOrderType("");
//		transRequest.setSystemSource("mimosa");
//		transRequest.setOrderNo(orderNo);
//		transRequest.setRelationProductNo("");
//		transRequest.setDirection(direction);
//		transRequest.setInputAccountNo(inputAccountNo);
//		transRequest.setOutpuptAccountNo(outpuptAccountNo);
